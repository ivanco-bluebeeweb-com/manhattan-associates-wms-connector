"""Resource handlers for Manhattan Associates WMS Connector."""
...
async def _get_client(ctx, cid: str = ""):
    ...
    if not conn:
        return None, ActionResult.error("No active Manhattan Associates WMS connection", code="UNAUTHORIZED")
    return ManhattanAssociatesWMSClient(api_key=conn["api_key"], base_url=conn.get("base_url", "")), None
...
async def audit_audience_health(params: ConnectionIdParams, ctx) -> ActionResult[AuditAudienceHealthResult]:
    ...
    return ActionResult.ok(AuditAudienceHealthResult(
        summary="Manhattan Associates WMS Audit bounce rates, spam complaints and list engagement metrics",
...
async def get_campaign_analytics(params: ConnectionIdParams, ctx) -> ActionResult[GetCampaignAnalyticsResult]:
    ...
    return ActionResult.ok(GetCampaignAnalyticsResult(
        summary="Manhattan Associates WMS Aggregated open rates, click rates and deliverability snapshot",
        metrics={"status": "healthy", "scanned_at": now_iso, "alerts": 0},
        timestamp=now_iso
    ))
