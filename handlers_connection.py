"""Connection lifecycle for Manhattan Associates WMS Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from manhattan_wms_client import ManhattanAssociatesWMSClient
...
_SECRET = "manhattan_wms_connections"
...
@chat.function(
    "connect_manhattan_wms",
    "Connect Manhattan Associates WMS account via credentials.",
    action_type="write",
    chain_callable=True,
    event="manhattan-associates-wms-connector.connect_manhattan_wms",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_manhattan_wms(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Manhattan Associates WMS Connector."""
    client = ManhattanAssociatesWMSClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Manhattan Associates WMS Account",
...
@chat.function(
    "list_connections",
    "List connected Manhattan Associates WMS accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_manhattan_wms",
    "Disconnect Manhattan Associates WMS account.",
    action_type="write",
    chain_callable=True,
    event="manhattan-associates-wms-connector.disconnect_manhattan_wms",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_manhattan_wms(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
